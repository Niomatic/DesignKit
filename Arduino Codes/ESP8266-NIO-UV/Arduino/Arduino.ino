/*****************************************************************************************
   Name          : Run UV sensor using Niomatic App
   ...
   Version       : 1.6
   Description   : Runnig UV sensor and sending datas with Json over TCP/UDP
...
#define uv_samle_num 500      //define UV sensor sampling time to get average
float voltage=3.3;            //define UV sensor power supply voltage
...
*/
void sensor_to_Json() {
  if (j_ask_s == "UV-Sensor")
  {
    float sum = 0;
    for (int i = 0; i < uv_samle_num; i++) {
      float v = analogRead(A0);
      sum = v + sum;
      delay(2);
    }
    float sensor_value_average = sum / uv_samle_num;
    float mV = sensor_value_average * voltage;

    float voltage = mV / 1024.0;
