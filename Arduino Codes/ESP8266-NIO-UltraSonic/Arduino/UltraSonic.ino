/*****************************************************************************************
   Name          : Run Ultrasonic sensor using Niomatic App
   ...
   Version       : 1.6
   Description   : Runnig Ultrasonic sensor and sending datas with Json over TCP/UDP
...
******************************************************************************************
  Ultrasonic Sensor
  HC SR-04 requires 5V Vcc to function

  Wiring :
  Esp8266 has 3.3V tolerant pins. So we have connected Echo pin of HC SR-04 to Esp8266 digital pin D6 via resistor network formed using (2.2K and 4.7K).
  2.2K is connected to Echo pin and 4.7K is grounded.
   trigPin = 14    //Trig - Ultrasonic sensor connected to NodeMCU GPIO14 i.e.D5
   echoPin = 12    //Echo - Ultrasonic sensor connected to NodeMCU GPIO12 i.e.D6
...
void sensor_to_Json() {
  if (j_ask_s == "Ultrasound-Sensor")
  {
    ...
    */
    ...
    long duration = pulseIn(echoPin, HIGH);// convert the time into a distance
    dist_cm = (duration / 2) / 29.1;
    String distance=(String)dist_cm;
    dist_cm = map(dist_cm, 0, 400, 0, 100);

    StaticJsonBuffer<200> jsonBuffer;
    JsonObject& root = jsonBuffer.createObject();
    root["identify"] = j_ask_s;
    root["Value-distance"] = distance;
    root["Value-time"] = duration;
